            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // Go through the _nodes vector with each element represented by a smart pointer.
                        // create new element if ID does not yet exist. newNode is the last element, or the first if the last parameter comes back true.
                        if (newNode == _nodes.end())    // if the node isnt in the vector.
                        {
                            _nodes.emplace_back(std::make_unique<GraphNode>(id)); //then create the element.
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            edge->SetChildNode((*childNode).get()); // Use .get() because of smart pointer.
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get());  // reference for parent.
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // ownership for child.
                        }
